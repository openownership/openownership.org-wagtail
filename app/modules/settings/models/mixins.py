...
from modules.settings.blocks import footer_nav_blocks, navbar_blocks, social_media_blocks
...
class AnalyticsSettings(BaseSiteSetting):
    ...
    analytics_property_id = models.CharField(  # noqa: DJ001
    ...
    tag_manager_property_id = models.CharField(  # noqa: DJ001
...
class MetaTagSettings(BaseSiteSetting):
    ...
    meta_description = models.TextField(  # noqa: DJ001
...
class SocialMediaSettings(BaseSiteSetting):
    ...
    def get_social_context(cls, site):
        ...
        # Only trust a cache entry that actually holds social links. An entry with
        # none is stale (e.g. cached before any accounts were added) and would hide
        # the links site-wide, so rebuild instead. Empty builds are never cached, so
        # the links self-heal on the first request after they are set up.
        if cached and cached.get("social_links"):
            return cached

        data = cls.for_site(site).build_social()
        ...
        for sub_block in block.value["objects"]:
            if sub_block.block_type == "nav_item":
                ...
                for link in sub_block.value.get("links"):
                    sub_nav_objects.append(  # noqa: PERF401
                        {
