...
then the regime-sub feed), gathers a `SyncReport`, and notifies Slack once at
the end.
"""
...
# Order matters: countries are the parent of commitments and regimes, and the
# regime-sub feed augments regimes that the regime syncer has created.
SYNCERS = [CountrySyncer, CommitmentSyncer, RegimeSyncer, RegimeSubSyncer]
...
def _notify(report: SyncReport) -> None:
    ...
    if report.has_failures:
        notionbot.fail(subject, report.summary())
    else:
