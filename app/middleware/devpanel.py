"""
This middleware was originally this package: https://github.com/loftylabs/django-developer-panel
However that package has long been abandoned so we drop this middleware in in order to use
the devpanel in Firefox available here: https://github.com/drcongo/djdevpanel-devtools
"""
...
def debug_payload(request, response, view_data):
    ...
    s = SafeExceptionReporterFilter().get_safe_settings()
    ...
    payload_script = "<script>var dj_chrome = {};</script>".format(
        json.dumps(payload, cls=LazyEncoder)
    )
    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if cbv:
            ...
            for member in inspect.getmembers(view_func.view_class):
                ...
                if (
                    member[0] in VIEW_METHOD_WHITEIST
                    and member[0] not in PATCHED_METHODS[klass]
                ):
    ...
    def process_response(self, request, response):
        ...
        if len(bits) > 1:
            try:
                ...
                console.warn("Failed to deliver djdevpanel data")
